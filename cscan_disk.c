...
int main()
{
    ...
    if (direction == 1)
    {
        int largest = arr[n - 1] + (100 - arr[n - 1] % 100) - 1;

        int smallest = arr[findMin(arr, head, n)];
        seek_op = largest - head + smallest + largest;
        printf("\n Seek Operation are :%d ", seek_op);
    }
    else if (direction == 0)
    {
        int smallest = arr[findMin(arr, head, n) + 1];
        int largest = arr[n - 1] + (100 - arr[n - 1] % 100) - 1;
        seek_op = head + largest + (largest - smallest);
        printf("\n Seek Operation are :%d ", seek_op);
    }
    else
